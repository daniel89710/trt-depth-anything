  ...
  const size_t max_workspace_size)
{
  ...
  if (precision == "int8") {
    ...
    if (build_config.calib_type_str == "Entropy") {
      calibrator.reset(
        new tensorrt_depth_anything::Int8EntropyCalibrator(stream, calibration_table, norm_factor_));
      ...
      calibrator.reset(
        new tensorrt_depth_anything::Int8MinMaxCalibrator(stream, calibration_table, norm_factor_));
    }
...
void TrtDepth_Anything::preprocessGpu(const std::vector<cv::Mat> & images)
{
...
bool TrtDepth_Anything::doInference(const std::vector<cv::Mat> & images, ObjectArrays & objects)
{
  if (!trt_common_->isInitialized()) {
    return false;
  }
...
void TrtDepth_Anything::preprocessWithRoiGpu(
...
void TrtDepth_Anything::preprocessWithRoi(
  const std::vector<cv::Mat> & images, const std::vector<cv::Rect> & rois)
{
  const auto batch_size = images.size();
  auto input_dims = trt_common_->getBindingDimensions(0);
...
void TrtDepth_Anything::multiScalePreprocessGpu(const cv::Mat & image, const std::vector<cv::Rect> & rois)
{
...
void TrtDepth_Anything::multiScalePreprocess(const cv::Mat & image, const std::vector<cv::Rect> & rois)
{
  const auto batch_size = rois.size();
  auto input_dims = trt_common_->getBindingDimensions(0);
  input_dims.d[0] = batch_size;
...
bool TrtDepth_Anything::doInferenceWithRoi(
...
bool TrtDepth_Anything::doMultiScaleInference(
...
// This method is assumed to be called when specified DEPTH_ANYTHING model contains
// EfficientNMS_TRT module.
bool TrtDepth_Anything::feedforward(const std::vector<cv::Mat> & images, ObjectArrays & objects)
{
...
bool TrtDepth_Anything::feedforwardAndDecode(const std::vector<cv::Mat> & images, ObjectArrays & objects)
{
  ...
  const auto output_dims = trt_common_->getBindingDimensions(1); 
  cv::Mat depthImage(output_dims.d[2], output_dims.d[3], CV_32FC1, out_prob_h_.get());
...
cv::Mat TrtDepth_Anything::getDepthImage(const std::string cFormat)
{
  const auto output_dims = trt_common_->getBindingDimensions(1);
  ...
  cv::Mat ret;
  //depthImage.convertTo(ret, CV_8U);
  //  return ret;  
  cv::normalize(depthImage, depthImage, 0, 255, cv::NORM_MINMAX, CV_8U);
...
// This method is assumed to be called when specified DEPTH_ANYTHING model contains
// EfficientNMS_TRT module.
bool TrtDepth_Anything::multiScaleFeedforward(const cv::Mat & image, int batch_size, ObjectArrays & objects)
{
  std::vector<void *> buffers = {
    input_d_.get(), out_num_detections_d_.get(), out_boxes_d_.get(), out_scores_d_.get(),
    out_classes_d_.get()};
...
bool TrtDepth_Anything::multiScaleFeedforwardAndDecode(
...
void TrtDepth_Anything::decodeOutputs(
  float * prob, ObjectArray & objects, float scale, cv::Size & img_size) const
{
  ...
  generateDepth_AnythingProposals(grid_strides, prob, score_threshold_, proposals);
...
void TrtDepth_Anything::generateGridsAndStride(
...
void TrtDepth_Anything::generateDepth_AnythingProposals(
  ...
  ObjectArray & objects) const
{
  ...
  for (int anchor_idx = 0; anchor_idx < num_anchors; anchor_idx++) {
    ...
    // depth_anything/models/yolo_head.py decode logic
    // To apply this logic, DEPTH_ANYTHING head must output raw value
...
void TrtDepth_Anything::qsortDescentInplace(ObjectArray & face_objects, int left, int right) const
{
  int i = left;
  int j = right;
...
void TrtDepth_Anything::nmsSortedBboxes(
...
cv::Mat TrtDepth_Anything::getMaskImageGpu(float *d_prob, nvinfer1::Dims dims, int out_w, int out_h, int b)
{
  //NCHW
...
cv::Mat TrtDepth_Anything::getMaskImage(float *prob, nvinfer1::Dims dims, int out_w, int out_h)
{
  //NCHW
  int classes = dims.d[1];
  int height = dims.d[2];
...
int TrtDepth_Anything::getMultitaskNum(void)
{
  return multitask_;
}

cv::Mat TrtDepth_Anything::getColorizedMask(int index, std::vector<Colormap> &colormap)
{
  cv::Mat mask;
...
cv::Mat TrtDepth_Anything::getSpecifiedMask(int index, unsigned char specified)
{
  cv::Mat mask;
  mask = segmentation_masks_[index];
  int width = mask.cols;
